...
def pq_anna(spark, file_path):
    ...
    anna_qq = spark.sql('SELECT * FROM people WHERE first_name= "Anna" AND income >= 7000')
...
def main(spark, file_path):
    ...
    print(f'Times to run pq_anna Query 25 times on {file_path}')
    print(times)
    times.sort()
    print(f'Maximum Time taken to run pq_anna Query 25 times on {file_path}:{max(times)}')
    print(f'Minimum Time taken to run pq_anna Query 25 times on {file_path}:{min(times)}')
    print(f'Median Time taken to run pq_anna Query 25 times on {file_path}:{(times[len(times)//2] + times[~(len(times)//2)])/2}')
