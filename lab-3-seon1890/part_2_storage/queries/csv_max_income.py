...
def main(spark, file_path):
    ...
    print(f'Times to run csv_max_income times on {file_path}')
    print(times)
    times.sort()
    print(f'Maximum Time taken to run csv_max_income 25 times on {file_path}:{max(times)}')
    print(f'Median Time taken to run csv_max_income Query 25 times on {file_path}:{(times[len(times)//2] + times[~(len(times)//2)])/2}')
    print(f'Minimum Time taken to run csv_max_income 25 times on {file_path}:{min(times)}')
