''' 
$ spark-submit evaluation_large.py bt2227 20 0.1
'''
...
def main(spark, rank, reg, netID):
    train = spark.read.parquet(f'hdfs:/user/{netID}/train_large.parquet')
    ...
    model = als.fit(train)
    ptemp = "models/ALS_rank_{}_reg_{}".format(rank, reg)
    als_path = ptemp + "/als"
    print("Saving model...")
    ...
    # RMSE
    validation = spark.read.parquet(f'hdfs:/user/{netID}/vals_large.parquet')
...
if __name__ == "__main__":
    memory = "8g"
    spark = SparkSession.builder.appName("Evaluate Large").config("spark.executor.memory", memory).config("spark.driver.memory", memory).getOrCreate()
    netID = sys.argv[1]
    rank = int(sys.argv[2])
