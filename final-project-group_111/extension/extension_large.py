'''
$ spark-submit extension_large.py bt2227 20 0.1
'''
...
def main(spark, rank, reg, netID):
    ...
    test = spark.read.parquet(f'hdfs:/user/{netID}/test_large.parquet')
...
if __name__ == "__main__":
    memory = "8g"
    spark = SparkSession.builder.appName("Extension Large").master('yarn').config("spark.executor.memory", memory).config("spark.driver.memory", memory).getOrCreate()
    path = sys.argv[1]
    rank = int(sys.argv[2])
